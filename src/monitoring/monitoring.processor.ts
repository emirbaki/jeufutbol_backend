...
export class MonitoringProcessor extends WorkerHost {
  ...
  async process(
    job: Job<
      FetchProfileTweetsJobData | RefreshAllProfilesJobData,
      any,
      string
    >,
  ): Promise<any> {
  ...
  private async handleFetchProfileTweets(
    ...
    // Fetch tweets
    const tweetsCount =
      await this.monitoringService.fetchAndStoreTweetsInternal(
        profileId,
        count,
      );
    ...
    if (tweetsCount > 0) {
      try {
        ...
        const indexedCount =
          await this.aiInsightsService.indexTweetsToVectorDb(profileId);
        this.logger.log(
          `Auto-indexed ${indexedCount} tweets to vector DB for profile ${profileId}`,
        );
      } catch (error) {
  ...
  private async handleRefreshAllProfiles(
    ...
    const enqueuedCount =
      await this.monitoringService.refreshAllProfilesInternal();
